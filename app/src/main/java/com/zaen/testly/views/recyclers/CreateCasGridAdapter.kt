...
class CreateCasGridAdapter(private val casList: ArrayList<FirebaseDocument>) : CreateCasAdapter(casList) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        var view: View? = null
        val inflater = LayoutInflater.from(parent.context)
        when(viewType){
            CARD     -> {
                view = inflater.inflate(R.layout.item_recycler_grid_create_card,parent,false)
                return CardHolder(view)
            }
            SET -> {
                view = inflater.inflate(R.layout.item_recycler_grid_create_set,parent,false)
    ...
    class CardHolder(view: View) : RecyclerView.ViewHolder(view){
        ...
        private val dateText: TextView = view.findViewById(R.id.text_date_grid_create_card)
